...
class FlickrSlideshow {
    ...
    public function fshow( $atts ) {
        ...
        $simpleLink = sprintf('<a href="http://www.flickr.com/photos/%s/sets/%s/" target="_blank">'.__('Click to View','flickr_slideshow').'</a>',$username, $photosetid, $thumburl);
        $return = sprintf('<div style="max-width: %spx; height: %spx">',$atts['width'],$atts['height'])."\n";
        $return .= '<iframe src="'.$this->get_url($user_id,$atts['photosetid'],$atts['username']).'" style="width: 100%; height: '.$atts['height'].'px">'."\n";
        $return .= '</iframe>'."\n";
        $return .= '<noframes>'."\n";
        $return .= $simpleLink;
        $return .= '</noframes>'."\n";
